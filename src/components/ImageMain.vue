      ...
      <el-row :gutter="10">
        <el-col :span="6" :offset="0" v-for="(item, index) in list" :key="index">
          <el-card shadow="hover" class="relative mb-3" :body-style="{ padding: 0 }" :class="{ 'border-blue-500': item.checked }">
            <el-image :src="item.url" fit="cover" class="h-[150px]" style="width: 100%" :preview-src-list="[item.url]" :initial-index="0"></el-image>
            ...
            <div class="flex items-center justify-center p-2">
              ...
              <el-popconfirm title="是否删除该图片？" confirmButtonText="确认" cancelButtonText="取消" @confirm="handleDelete(item.id)">
                <template #reference>
...
import { Confirm, Message } from '@/utils.js'
...
const handleEdit = item => {
  Confirm('重命名', item.name).then(({ value }) => {
    loading.value = true
...
const handleChooseChange = item => {
  if (item.checked && checkedImage.value.length > props.limit) {
    item.checked = false
    return Message(`最多只能选中${props.limit}张`, 'error')
  }
